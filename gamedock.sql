...
CREATE TABLE `users` (
  ...
  `billing_card_name` varchar(100) DEFAULT NULL,
  `billing_card_last4` char(4) DEFAULT NULL,
  `billing_card_brand` varchar(20) DEFAULT NULL,
  `billing_card_expiry` varchar(5) DEFAULT NULL,
  PRIMARY KEY (`user_id`),
  UNIQUE KEY `email` (`email`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

INSERT INTO `users` (`user_id`, `fullname`, `email`, `phone`, `password`, `created_at`, `profile_pic`) VALUES
...
(4,1,'Tips for selling used Xbox games?','Looking to sell my collection of Xbox One games. Any advice on pricing and shipping? Has anyone sold through GameDock before?','2026-07-14 05:07:23'),
(5,1,'Starfield - worth the hype?','Thinking about renting Starfield to see if it lives up to all the buzz. 100+ hours of gameplay right? Let me know if you think it is worth the time investment.','2026-07-14 05:07:23'),
(6,1,'Best indie games of 2024','What are your favorite indie games released this year? I am tired of AAA titles and want to discover some hidden gems. Any recommendations?','2026-07-14 05:07:23'),
(7,1,'Cyberpunk 2077 still worth playing?','Just heard about the new patches. Is the game in a good place now? Thinking about renting it to check out the improvements.','2026-07-14 05:07:23');

CREATE TABLE `forum_reactions` (
  `reaction_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `post_id` int(10) unsigned NOT NULL,
  `user_id` int(10) unsigned NOT NULL,
  `reaction_type` enum('like','love','haha','wow','sad','angry','dislike') NOT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`reaction_id`),
  UNIQUE KEY `user_post` (`user_id`,`post_id`),
  KEY `idx_post_id` (`post_id`),
  CONSTRAINT `fk_reaction_post` FOREIGN KEY (`post_id`) REFERENCES `forum_posts` (`post_id`) ON DELETE CASCADE,
...
CREATE TABLE `cart_items` (
  ...
  `rental_days` int(10) unsigned NOT NULL DEFAULT 0,
  `added_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`cart_item_id`),
  UNIQUE KEY `user_game_days` (`user_id`,`game_id`,`rental_days`),
  KEY `idx_cart_user_id` (`user_id`),
  KEY `fk_cart_game` (`game_id`),
  CONSTRAINT `fk_cart_game` FOREIGN KEY (`game_id`) REFERENCES `games` (`game_id`) ON DELETE CASCADE,
